...
export const SENSITIVITIES = {
  low: {
    label: "Low",
    impact: 3.5,
    stillnessWindow: 2000,
    stillnessMax: 0.25,
    description: "Fewer alerts, only major falls",
  },
  medium: {
    label: "Medium",
    impact: 2.5,
    stillnessWindow: 1800,
    stillnessMax: 0.3,
  ...
  high: {
    label: "High",
    impact: 2.0,
    stillnessWindow: 1500,
    stillnessMax: 0.38,
    description: "Very sensitive, may have false alarms",
  },
};
...
const FallDetectionContext = createContext<FallDetectionContextValue | null>(
  null,
);
...
export function FallDetectionProvider({ children }: { children: ReactNode }) {
  ...
  const [accel, setAccel] = useState<AccelData>({
    x: 0,
    y: 0,
    z: 0,
    magnitude: 1,
  ...
  // ── WebSocket connection (receives fall from pose engine on laptop) ──
useEffect(() => {
  const wsUrl = `${base.replace('https://', 'wss://').replace('http://', 'ws://')}?patientId=PAT-HEWR25L`;
  const ws = new WebSocket(wsUrl);

  ws.onopen = () => console.log('[WS] Mobile app connected to room');

  ws.onmessage = (e) => {
    try {
      const msg = JSON.parse(e.data);
      if (msg.type === 'VISION_CONFIRMED' && modeRef.current === 'monitor' && isActiveRef.current) {
        triggerFallAlert(
  ...
  ws.onclose = () => console.log('[WS] Mobile app disconnected');
  ...
  useEffect(() => {
    ...
    if (!isActive) {
      if (phoneAccelSubRef.current) {
        phoneAccelSubRef.current.remove();
        phoneAccelSubRef.current = null;
      }
      return;
    }
    Accelerometer.setUpdateInterval(100);
    phoneAccelSubRef.current = Accelerometer.addListener(
  ...
  function processFallAlgorithm(accelData: AccelData, now: number) {
    ...
    if (win.type === "impact") {
      ...
      const variance =
        readings.reduce((s, v) => s + Math.pow(v - mean, 2), 0) /
        readings.length;
      const stdDev = Math.sqrt(variance);

      if (stdDev < thresh.stillnessMax && modeRef.current === "monitor") {
        watchFallRef.current = {
          triggered: true,
          time: Date.now(),
          magnitude: win.data.magnitude,
        };
  ...
  function checkCorrelation(source: "watch" | "pose", data: AccelData) {
    ...
    const watchRecent =
      watch.triggered && now - watch.time < CORRELATION_WINDOW_MS;
    const poseRecent =
      pose.triggered && now - pose.time < CORRELATION_WINDOW_MS;

    if (watchRecent && poseRecent && !correlationFiredRef.current) {
      correlationFiredRef.current = true;
      triggerFallAlert(
        { x: 0, y: 0, z: 0, magnitude: watch.magnitude },
        pose.confidence,
        pose.feature,
        "both",
      );
      ...
      setTimeout(() => {
        if (modeRef.current !== "monitor" || correlationFiredRef.current)
          return;
        const wStill =
          watchFallRef.current.triggered &&
          Date.now() - watchFallRef.current.time < CORRELATION_WINDOW_MS * 2;
        const pStill =
          poseFallRef.current.triggered &&
          Date.now() - poseFallRef.current.time < CORRELATION_WINDOW_MS * 2;
        if (source === "watch" && wStill && !pStill) {
          triggerFallAlert(
            { x: 0, y: 0, z: 0, magnitude: watchFallRef.current.magnitude },
            0.65,
            "watch_impact_unconfirmed",
            "watch",
          );
          watchFallRef.current = { triggered: false, time: 0, magnitude: 0 };
        }
        if (source === "pose" && pStill && !wStill) {
          triggerFallAlert(
            { x: 0, y: 0, z: 0, magnitude: 0 },
            poseFallRef.current.confidence,
            poseFallRef.current.feature + "_unconfirmed",
            "skeleton",
          );
          poseFallRef.current = {
            triggered: false,
  ...
  async function confirmFall() {
    ...
    // Patch event status AND trigger Twilio in one go
    if (pendingEventId) {
    ...
    // ✅ Trigger Twilio alert directly from confirmed fall
    fetch(`${base}api/patient/fall-alert`, {
      // ← your Twilio endpoint
      method: "POST",
      headers: { "Content-Type": "application/json", ...authHeader() },
      body: JSON.stringify({ patientId: "PAT-HEWR25L" }),
    })
      .then(() => console.log("[Twilio] Alert sent"))
      .catch((err) => console.error("[Twilio] Alert failed:", err));
  ...
  async function sendManualAlert() {
    ...
    try {
      if (Platform.OS !== "web") {
        ...
        if (status === "granted") {
          ...
          locationStr = `${locationLat.toFixed(5)}, ${locationLng.toFixed(5)}`;
          mapsLink = `https://maps.google.com/?q=${locationLat.toFixed(5)},${locationLng.toFixed(5)}`;
        }
      }
    } catch {}
    ...
    const confidencePct = Math.round(fallConfidence * 100);
    const sourceText =
      fallSource === "both"
        ? "Watch + Camera"
        : fallSource === "watch"
          ...
          : "Camera skeleton";
    const smsBody =
      `🚨 FALL ALERT\nPatient: ${patientName} (${patientId}) has fallen and may need help.\n` +
      `Detected by: ${sourceText} (${confidencePct}% confidence)\n` +
    ...
    // Fire server-side alert: Twilio SMS to all contacts + push to all care givers
    fetch(`${base}api/patient/fall-alert`, {
      method: "POST",
      headers: { "Content-Type": "application/json", ...authHeader() },
      body: JSON.stringify({
    ...
    // Also open the SMS app locally on the phone for each contact
      ...
      >
        <View style={s.alertScreen}>
          ...
          <Text style={s.alertSub}>
            Detected via: {sourceLabel[fallSource]}
          </Text>
        ...
        <View style={s.resultOverlay}>
          <View style={s.resultSheet}>
            <View style={s.resultIconWrap}>
              <Ionicons
                name={sourceIcon[fallSource] as any}
                size={48}
                color={Colors.danger}
              />
            </View>
            ...
            <View style={s.confidenceBadge}>
              <Text style={s.confidenceText}>
                {Math.round(fallConfidence * 100)}% confidence ·{" "}
                {sourceLabel[fallSource]}
              </Text>
            </View>
            ...
            {!alertSent ? (
              <Pressable
                style={[s.emergencyBtn, alertSending && { opacity: 0.7 }]}
                onPress={sendManualAlert}
              ...
              <View style={s.confirmedBanner}>
                ...
                <Text style={s.confirmedText}>
                  Alert sent — SMS opened for all emergency contacts with your location and fall details.
                </Text>
              </View>
            )}
            <Pressable style={s.falseAlarmBtn} onPress={markFalseAlarm}>
              <Text style={s.falseAlarmText}>
                I'm OK — This Was a False Alarm
              </Text>
            </Pressable>
...
export function useFallDetection(): FallDetectionContextValue {
  ...
  if (!ctx)
    throw new Error(
      "useFallDetection() must be inside <FallDetectionProvider>",
    );
  return ctx;
}
